# Coding Standards — onecx-permission-ui

In this section, the term _permission_ is used as main business topic (feature) in this project.
...
src/app/
  ...
  onecx-*permission*-remote.module.ts  # Module Federation shell bootstrap
  ...
  *permission*/                   # Root directory for business components (feature)
    *permission*.module.ts        # Lazy-loaded feature module
    *permission*-search/          # Smart container — list all permissions
    *permission*-detail/          # Smart container — view/edit one permission
    *permission*-delete/          # Dialog: confirm delete
src/assets/                  # Assets
...
- Component selectors: `app-` prefix, kebab-case element (`app-permission-search`)
...
// data
private readonly dataSubject$ = new BehaviorSubject<Permission[]>([])
public data$: Observable<Permission[]> = this.dataSubject$.asObservable()
// dialog
...
catchError((err) => {
  this.exceptionKey = 'EXCEPTIONS.HTTP_STATUS_' + Utils.mapping_error_status(err.status) + '.PERMISSION'
